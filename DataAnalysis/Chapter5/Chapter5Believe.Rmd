...
laplace <- function(Period,q,t,s)(q*exp(-(abs(Period-t)/s)))
```
...
#NLLS: 
model <- nls(Emood2 ~ laplace(Period,q,t,s), data=ex.datamoodfirst, start=list(q=0.43,t=1572, s=200))
summary(model) #q=1.1, r=1548, s=128
...
modelbis <- nls(Emood2 ~ laplace(Period,q,t,s), data=ex.datamoodfirst, start=list(q=0.43,t=1572, s=200), upper=list(q=1,t=1900,s=500), algorithm = "port")
summary(modelbis) #q = 1, r = 1542, s = 142
...
modelother <- nls(Emood2 ~ laplace(Period,q,t,s), data=ex.datamoodother, start=list(q=0.7,t=1549,s=370),upper=list(q=1,t=1900,s=500), algorithm = "port")
summary(modelother)
#q=0.68, r=1551, s=398
```
...
```{r}
modelc <- nls(Complementizer2 ~ laplace(Period,q,t,s), data=ex.datacfirstind, start=list(q=0.44,t=1589, s=153), upper=list(q=1,t=1900,s=500), algorithm = "port")
summary(modelc) #q=0.45 t=1584 s=174
...
#modelco <- nls(Complementizer2 ~ laplace(Period,q,t,s), data=ex.datacotherind, start=list(q=0.3,t=1700, s=200))
#summary(modelco)
